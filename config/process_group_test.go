...
func TestGetAllGroup(t *testing.T) {
	...
	if len(groups) != 2 || !util.HasAllElements(util.StringArrayToInterfaceArray(groups), []interface{}{"group1", "group2"}) {
		t.Fail()
	}
...
func TestGetAllProcessInGroup(t *testing.T) {
	...
	if len(procs) != 2 || !util.HasAllElements(util.StringArrayToInterfaceArray(procs), []interface{}{"proc1_1", "proc1_2"}) {
		t.Fail()
	}
...
func TestRemoveFromGroup(t *testing.T) {
	...
	if len(procs) != 2 || !util.HasAllElements(util.StringArrayToInterfaceArray(procs), []interface{}{"proc2_2", "proc2_3"}) {
		t.Fail()
	}
